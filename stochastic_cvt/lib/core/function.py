...
def train_one_epoch(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        with autocast(enabled=config.AMP.ENABLED):
            ...
            outputs, attn_scores, sigmas = model(x)
    ...
    if writer_dict and comm.is_main_process():
        stochastic_depth = spec_.DEPTH[-1]
        ...
        for depth in range(stochastic_depth):
            writer.add_histogram(f'stg_3_layer_{depth}_attention_weights',
            attn_scores[depth], global_steps)
            writer.add_histogram(f'stg_3_layer_{depth}_sigmas',
            sigmas[depth], global_steps)
